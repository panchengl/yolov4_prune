...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='weights/last_voc_prune_ap43.pt', help='sparse model weights')
    parser.add_argument('--algorithm_type', type=str, default='v4', help='sparse model weights')
    ...
    parser.add_argument('--input_size', type=int, default=608, help='inference size (pixels)')
    ...
    model = Darknet(opt.cfg, img_size, opt.algorithm_type).to(device)

    if opt.weights.endswith(".pt"):
        model.load_state_dict(torch.load(opt.weights, map_location=device)['model'].state_dict())
    else:
        _ = load_darknet_weights(model, opt.weights)
    print('\nloaded weights from ', opt.weights)
    #
    # eval_model = lambda model: test(model=model, cfg=opt.cfg, data=opt.data, batch_size=16, img_size=img_size)
    eval_model = lambda model: test(cfg=opt.cfg, names_file=opt.names_classes, path=opt.val_path,
                                             batch_size=8,
                                             img_size=opt.input_size,
                                             conf_thres=0.01,
                                             save_json=False,
    ...
    #
    compact_model = Darknet([model.hyperparams.copy()] + compact_module_defs, img_size, opt.algorithm_type).to(device)
