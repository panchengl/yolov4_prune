...
def test(cfg,
    ...
    assert (model or path)
    if model is None:
        device = torch_utils.select_device(opt.device, batch_size)
        ...
        # Initialize model
        model = Darknet(cfg, img_size, type)
    ...
    # Configure run
    nc = 1 if single_cls else int(len(open(names_file).readlines()))  # number of classes
    names = load_classes(names_file)  # class names
    ...
    for i, c in enumerate(ap_class):
        maps[c] = ap[i]
        print("class {} ap is {}".format(i, ap[i]))
    # print("map is", map)
    return (mp, mr, map, mf1, *(loss.cpu() / len(dataloader)).tolist()), maps, t
...
if __name__ == '__main__':
    ...
    parser.add_argument('--names_file', type=str, default='data/coco.names', help='*.data path')
