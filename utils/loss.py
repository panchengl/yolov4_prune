...
def compute_loss(preds, targets, model):
    ...
    for i, pi in enumerate(preds):
        ...
        if nb :
            ...
            # pred_wh = torch.sigmoid(ps[:, 2:4]).clamp(max=1E3) * anchor_vec[i]  # anchor_vec:scale in ori_img; feature_map_wh*scale->ori_wh
            ...
            giou = bbox_iou(pred_box.t(), target_box[i], x1y1x2y2=False, GIoU=True) #
