...
from utils.optimizer import APGNAG
...
last = wdir + 'last_voc_auto_prune.pt'
best = wdir + 'best_voc_auto_prune.pt'
...
hyp = {'giou': 3.54,  # giou loss gain
       ...
       'lrf': 0.0005,  # final learning rate (with cos scheduler)s
...
def train(hyp):
    ...
    if opt.sss:
        optimizer2 = APGNAG([{'params': model.lambda_block}], lr=hyp['lr0'], momentum=opt.momentum, gamma=opt.gamma_data)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if not opt.notest or final_epoch:  # Calculate mAP
            results, maps, times = test.test(cfg=opt.cfg,
                                             ...
                                             save_dir=wdir)
        if opt.sss:
            print("lambda '{}'\n".format(model.module.lambda_block if hasattr(model, 'module') else model.lambda_block ))
        if epoch % opt.interval_prune == 0:
            ...
            CBL_idx, Conv_idx, prune_idx, _, _ = parse_module_defs2(model.module.module_defs if hasattr(model, 'module') else model.module_defs)
            # norm_thr = get_global_norm_thr(model, prune_idx, opt.global_percent)
            norm_thr_list, norm_prune_index = get_layer_norm_thr(model, prune_idx, opt.global_percent)
            print("norm_thr_list is", norm_thr_list)
            # print("norm index is", norm_prune_index)
            num_filters_l2, filters_mask_l2 = obtain_filters_mask_norm_per_layer(model, norm_thr_list, CBL_idx, prune_idx, layer_keep=opt.layer_keep)
            CBLidx2mask = {idx: mask for idx, mask in zip(CBL_idx, filters_mask_l2)}
            CBLidx2filters = {idx: filters for idx, filters in zip(CBL_idx, num_filters_l2)}
            for i in model.module.module_defs if hasattr(model, 'module') else model.module_defs:
                if i['type'] == 'shortcut':
                    i['is_access'] = False
            # print('merge the mask of layers connected to shortcut!')
            merge_mask(model.module if hasattr(model, 'module') else model, CBLidx2mask, CBLidx2filters)
            prune_soft_model_code(model.module if hasattr(model, 'module') else model, CBL_idx, CBLidx2mask)
            print("after soft prune, map test ")
            results_sfp, maps_sfp, times_sfp = test.test(cfg=opt.cfg,
                                             names_file=opt.names_classes,
                                             batch_size=8,
                                             img_size=opt.input_size,
                                             conf_thres=0.01,
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--weights', type=str, default='/home/pcl/pytorch_work/my_github/yolov3_pcl/yolov4_pcl/weights/best_yolov4_ap86.pt', help='initial weights path')
    parser.add_argument('--weights', type=str, default='weights/yolov4.weights', help='initial weights path')
    parser.add_argument('--resume', action='store_true', help='resume training from last.pt')
    parser.add_argument('--sr_flag', default=False, help='prune flag')
    parser.add_argument('--gamma', type=float, default=1e-4, help='prune flag')
    parser.add_argument('--sfp', type=bool, default=True, help='prune flag')
    ...
    parser.add_argument('--interval_prune', type=int, default=5, help='channel keep percent per layer')
    parser.add_argument('--gamma_data', type=float, default=0.8, metavar='G', help='gamma (default: 0.01)')
    parser.add_argument('--momentum', type=float, default=0.9, metavar='G', help='momentum (default: 0.9)')
    parser.add_argument('--sss', default=True, metavar='G', help='momentum (default: 0.9)')
